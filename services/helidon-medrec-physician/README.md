...
```
java -jar target/helidon-medrec-physician.jar
```
...
```
docker build -t helidon-medrec-physician .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 helidon-medrec-physician:latest
```
...
kubectl get service helidon-medrec-physician  # Get service info
```
...
```
./target/helidon-medrec-physician
```
...
```
docker build -t helidon-medrec-physician-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 helidon-medrec-physician-native:latest
```